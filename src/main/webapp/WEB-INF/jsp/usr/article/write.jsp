...
<script type="text/javascript">
	...
	function updateContentFields() {
        var selectedBoardId = document.querySelector('select[name="boardId"]').value;
        var contentRow = document.getElementById('content-row');

        if (selectedBoardId === "1") {
            // 공지사항 선택 시
            contentRow.innerHTML = '<textarea name="body" placeholder="공지사항 내용을 입력해" style="width: 100%; height: 200px;"></textarea>';
        } else if (selectedBoardId === "2") {
            // 자유 선택 시
            contentRow.innerHTML = '<textarea name="body" placeholder="자유 게시판 내용을 입력해" style="width: 100%; height: 200px;"></textarea>';
        } else if (selectedBoardId === "3") {
            // 질의응답 선택 시
            contentRow.innerHTML = `
                <textarea id="questionTopic" name="body" placeholder="질문을 입력해" style="width: 100%; height: 200px;"></textarea>
                <div id="suggestedAnswer" style="margin-top: 20px; color: gray;"></div>
            `;
        } else {
            contentRow.innerHTML = '<div class="toast-ui-editor"><script type="text/x-template"></script></div>';
        }
    }
